      ...
      ">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
          ...
          <div style="flex:1;min-width:0;overflow:hidden">
            <div style="font-weight:600;font-size:.95rem;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(d.clientName)}</div>
            <div style="font-size:.75rem;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(d.description)}</div>
          </div>
        </div>

        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <div style="font-size:.75rem;color:var(--text-muted)">Pagado: ${fmt(d.paidAmount)} · Hace ${age}d</div>
          <div style="font-size:.75rem;color:var(--text-muted)">${pct}%</div>
        </div>
        <div style="height:5px;background:var(--border-light);border-radius:99px;overflow:hidden;margin-bottom:8px">
          <div style="height:100%;width:${pct}%;background:${d.isPaid ? 'var(--success)' : 'var(--rose-gold)'};border-radius:99px"></div>
        </div>

        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <div style="font-size:.75rem;color:var(--text-muted)">${d.payments.length} pago${d.payments.length !== 1 ? 's' : ''} registrado${d.payments.length !== 1 ? 's' : ''}</div>
          <div>
            <div style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;font-weight:700;color:${d.isPaid ? 'var(--success)' : 'var(--danger)'};text-align:right">${d.isPaid ? '✓ ' : ''}${fmt(remaining)}</div>
            <div style="font-size:.7rem;color:var(--text-muted);text-align:right">de ${fmt(d.totalAmount)}</div>
          </div>
        </div>

        <div style="display:flex;gap:6px;flex-wrap:wrap;padding-top:10px;border-top:1px solid var(--border-light)">
          ${!d.isPaid ? `
          ...
          ` : `
            <span class="badge badge-success" style="padding:6px 12px">✓ Cancelada</span>
          `}
