  ...
  return debts.map((d, i) => {
    ...
    return `
      ...
      ">
        ...
        <div style="background:var(--bg-dark);border-radius:10px;padding:10px;margin-bottom:10px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:.8rem;font-weight:700;color:${d.isPaid ? 'var(--success)' : 'var(--danger)'}">${d.isPaid ? '✓ ' : ''}${fmt(remaining)}</span>
            <span style="font-size:.75rem;color:var(--text-muted)">de ${fmt(d.totalAmount)}</span>
          </div>
          ...
          <div style="height:5px;background:var(--border-light);border-radius:99px;overflow:hidden;margin-bottom:6px">
            <div style="height:100%;width:${pct}%;background:${d.isPaid ? 'var(--success)' : 'var(--rose-gold)'};border-radius:99px"></div>
          </div>
          <div style="font-size:.72rem;color:var(--text-muted)">${d.payments.length} pago${d.payments.length !== 1 ? 's' : ''} registrado${d.payments.length !== 1 ? 's' : ''}</div>
        </div>
